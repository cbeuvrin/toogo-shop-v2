-- Add features field to get_public_store_data function
-- This is needed so the live store can filter products by features (like "new_arrival")
...
DECLARE
  ...
  result jsonb;
BEGIN
  -- Find active tenant by ID for demo mode
  SELECT id, name, primary_host, extra_hosts, plan, status
  INTO t_record
  FROM tenants
  ...
  -- Same structure as get_public_store_data
  result := jsonb_build_object(
    'ok', true,
    'tenant', jsonb_build_object(
      'id', t_record.id,
