-- Expose the tenant's ACTIVE payment providers (OAuth) to anonymous storefront
-- visitors via the public RPCs. Until now the checkout read tenant_payment_integrations
-- and tenant_settings DIRECTLY, which RLS blocks for anonymous customers, so a store
-- that connected MercadoPago via OAuth showed "No hay métodos de pago configurados"
-- to its buyers (and the logo fell back to a placeholder for the same RLS reason).
--
-- Changes vs the previous definition (20260614120000):
--   * settings.mercadopago_public_key now COALESCEs the legacy tenant_settings key
--     with the OAuth integration's public_key (tenant_payment_integrations).
--   * settings.payment_providers: array of provider names that are active (OAuth),
--     so the checkout can offer them even when no legacy public key exists.
  ...
  result := jsonb_build_object(
    ...
    'settings', (
      SELECT jsonb_build_object(
        ...
        'mercadopago_public_key', COALESCE(
          ts.mercadopago_public_key,
          (SELECT tpi.public_key FROM tenant_payment_integrations tpi
            WHERE tpi.tenant_id = t_record.id AND tpi.provider = 'mercadopago' AND tpi.status = 'active'
            LIMIT 1)
        ),
        'paypal_client_id', ts.paypal_client_id,
        'whatsapp_number', ts.whatsapp_number,
        'payment_providers', (
          SELECT COALESCE(jsonb_agg(tpi.provider), '[]'::jsonb)
          FROM tenant_payment_integrations tpi
          WHERE tpi.tenant_id = t_record.id AND tpi.status = 'active'
        ),
