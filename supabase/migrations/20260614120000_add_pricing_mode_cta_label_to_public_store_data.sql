-- Expose service-product fields (pricing_mode, cta_label) to anonymous storefront
-- visitors via the public RPCs. Without these, the product modal/cards couldn't tell
-- a service is "Desde $X" (starting_from) nor show the custom WhatsApp button text,
-- so a service always rendered like a fixed-price product with "Consultar por WhatsApp".
-- Mirrors the pattern of template_id (RLS blocks direct table reads for anon; expose via SECURITY DEFINER RPC).
